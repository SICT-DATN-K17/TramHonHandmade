...
export default function AdminChatDetailPage() {
    ...
    const handleSendProposal = async () => {
        ...
        try {
            ...
            console.log('[Proposal] Chat reference_image:', chat?.reference_image);
            console.log('[Proposal] Creating product with payload:', productPayload);
            
            const customProduct = await axiosAuth.post('/products/', productPayload);
            console.log('[Proposal] Product created:', customProduct.data);
            console.log('[Proposal] Product image field:', customProduct.data.image);

            const formData = new FormData();
            // Send as ORDER_PROPOSAL message; backend will read `message` and optional `type`.
            formData.append('message', JSON.stringify(customProduct.data)); // send JSON string in message
            formData.append('type', 'ORDER_PROPOSAL');

            await axiosAuth.post(`/chat/${chatId}/send-message/`, formData);
            ...
            toast.error('Gửi đề xuất thất bại: ' + JSON.stringify(errorMsg));
        } finally {
            setIsSending(false);
        }
    };
