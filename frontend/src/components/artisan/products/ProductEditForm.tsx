...
export const ProductEditForm: React.FC<ProductEditFormProps> = ({ initialData }) => {
    ...
    if (!initialData) {
        return <div className="text-center p-8 text-gray-500">Không có dữ liệu sản phẩm.</div>
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-8 w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mb-2 truncate">Hiện tại: {initialData.name}</p>
                    <input id="name" required value={name} onChange={(e) => setName(e.target.value)} disabled={loading} placeholder="Tên sản phẩm mới" style={inputStyles} className="focus:ring-2 focus:ring-[#D96C39] focus:border-transparent focus:outline-none" />
                </div>

                <div>
                    <label htmlFor="price" style={labelStyles}>Giá (VNĐ)</label>
                    <p className="text-xs text-gray-500 mb-2">Hiện tại: {initialData.price.toLocaleString()} đ</p>
                    <input id="price" required value={price} onChange={(e) => setPrice(Number(e.target.value))} type="number" min="0" disabled={loading} placeholder="Giá mới" style={inputStyles} className="focus:ring-2 focus:ring-[#D96C39] focus:border-transparent focus:outline-none" />
                </div>

                <div>
                    <label htmlFor="stock" style={labelStyles}>Số lượng tồn kho</label>
                    <p className="text-xs text-gray-500 mb-2">Hiện tại: {initialData.stockQuantity}</p>
                    <input id="stock" required value={stockQuantity} onChange={(e) => setStockQuantity(Number(e.target.value))} type="number" min="0" disabled={loading} placeholder="Số lượng mới" style={inputStyles} className="focus:ring-2 focus:ring-[#D96C39] focus:border-transparent focus:outline-none" />
                </div>

                <div>
                    <label htmlFor="category" style={labelStyles}>Danh mục</label>
                    <p className="text-xs text-gray-500 mb-2">Hiện tại: {categories.find(c => c.categoryId === initialData.categoryId)?.categoryName || 'Trống'}</p>
                    <select
                        id="category"
                        disabled={loading}
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500 mb-2">Hiện tại: {initialData.status}</p>
                ...
                <div className="md:col-span-2">
                    ...
                    <p className="text-xs text-gray-500 mb-2 truncate">Hiện tại: {initialData.description}</p>
                ...
                <div className="md:col-span-2">
                    ...
                    <p className="text-xs text-gray-500 mb-2 truncate">Hiện tại: {initialData.image || 'Trống'}</p>
